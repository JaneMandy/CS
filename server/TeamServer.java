...
public class TeamServer extends Starter {
   protected int port;
   ...
   private final void A() {
      try {
         ...
         (new Accents(this.resources)).register(this.calls);
         ...
         String var3 = System.getProperty("cobaltstrike.server_bindto", "0.0.0.0");
         SecureServerSocket var4 = new SecureServerSocket(var3, this.port);
         CommonUtils.print_good("Team server is up on " + var3 + ":" + this.port);
         CommonUtils.print_info("SHA256 hash of SSL cert is: " + var4.fingerprint());
         super.initializeStarter(this.getClass());
         this.resources.call("listeners.go");

         while(true) {
            var4.acceptAndAuthenticate(this.pass, new PostAuthentication() {
         ...
         MudgeSanity.logException("team server startup", var5, false);
      }
   }
   ...
   public static void main(String[] var0) {
      ...
      if (var0.length != 0 && (var0.length != 1 || !"-h".equals(var0[0]) && !"--help".equals(var0[0]))) {
         if (var0.length != 2 && var0.length != 3 && var0.length != 4) {
            ...
            CommonUtils.print_error("The team server <host> must be an IP address. " + A);
         } else if ("127.0.0.1".equals(var0[0])) {
            CommonUtils.print_error("Don't use 127.0.0.1 for the team server <host>. " + A);
         } else if ("0.0.0.0".equals(var0[0])) {
            CommonUtils.print_error("Don't use 0.0.0.0 for the team server <host>. " + A);
