...
public class PreloaderEPS extends AbstractImagePreloader {
   ...
   public ImageInfo preloadImage(String uri, Source src, ImageContext context) throws IOException {
      if (!ImageUtil.hasImageInputStream(src)) {
         ...
         PreloaderEPS.EPSBinaryFileHeader binaryHeader = null;
   ...
   private PreloaderEPS.EPSBinaryFileHeader readBinaryFileHeader(ImageInputStream in) throws IOException {
      PreloaderEPS.EPSBinaryFileHeader offsets = new PreloaderEPS.EPSBinaryFileHeader();
   ...
   private boolean determineSize(ImageInputStream in, ImageContext context, ImageInfo info) throws IOException {
      ...
      try {
         ...
         size.setSizeInMillipoints((int)Math.round(bbox.getWidth() * 1000.0D), (int)Math.round(bbox.getHeight() * 1000.0D));
   ...
   static {
      EPS_BINARY_HEADER = PreloaderEPS.EPSBinaryFileHeader.class;
