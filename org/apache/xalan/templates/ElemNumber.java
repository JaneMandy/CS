...
public class ElemNumber extends ElemTemplateElement {
   ...
   XPath getCountMatchPattern(XPathContext support, int contextNode) throws TransformerException {
      ...
      if (null == countMatchPattern) {
         ...
         case 1:
            ElemNumber.MyPrefixResolver resolver;
            if (dtm.getNamespaceURI(contextNode) == null) {
               resolver = new ElemNumber.MyPrefixResolver(dtm.getNode(contextNode), dtm, contextNode, false);
            } else {
               resolver = new ElemNumber.MyPrefixResolver(dtm.getNode(contextNode), dtm, contextNode, true);
            }
   ...
   String formatNumberList(TransformerImpl transformer, long[] list, int contextNode) throws TransformerException {
      ...
      try {
         ...
         ElemNumber.NumberFormatStringTokenizer formatTokenizer = new ElemNumber.NumberFormatStringTokenizer(formatValue);
