...
public class Profile implements Serializable {
   ...
   public String getSSHPipeName() {
      String var1 = CommonUtils.pick(this.getString(".ssh_pipename"));
      return CommonUtils.replaceHashes(var1);
   }

   public String getPostExPipeName(String var1) {
      String var2 = CommonUtils.pick(this.getString(".post-ex.pipename"));
      var2 = CommonUtils.replaceHashes(var2);
      if (var2.length() > var1.length()) {
         var2 = var2.substring(0, var1.length());
      }

      return CommonUtils.pad(var2, '\u0000', var1.length());
   }

   public String getHeadersToRemove() {
      Set var1 = CommonUtils.toSet(this.getString(".headers_remove"));
      StringBuffer var2 = new StringBuffer();
      if (var1.size() == 0) {
         return "";
      } else if (this.exerciseCFGCaution()) {
         ...
         if (var2.toString().length() > 63) {
            CommonUtils.print_error(".headers_remove is " + var2.toString().length() + " characters in final form. The max is 63 characters. Please trim!");
            return "";
         } else {
   ...
   public boolean isEmptyString(String var1) {
      if (!this.hasString(var1)) {
         return true;
      } else {
         return this.getString(var1).length() == 0;
      }
   }
   ...
   public byte[] getStringAsBytes(String var1) {
      ...
      return var2 != null && !"".equals(var2) ? CommonUtils.toBytes(var2) : new byte[0];
   }
