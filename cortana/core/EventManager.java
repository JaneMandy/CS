...
public class EventManager {
   ...
   public void addListener(String var1, SleepClosure var2, boolean var3) {
      synchronized(this) {
         ...
         this.getListener(var1).add(new EventManager._A(var2, var3));
      }
   }
   ...
   protected List getListeners(String var1, ScriptInstance var2) {
      ...
      synchronized(this) {
         ...
         while(true) {
            while(var6.hasNext()) {
               EventManager._A var7 = (EventManager._A)var6.next();
               if (!var7.A().getOwner().isLoaded()) {
                  var6.remove();
               } else if (var3 == null || var3.equals(var7.A().getOwner().getMetadata().get("%scriptid%"))) {
                  var5.add(var7.A());
                  if (var7.B()) {
                     var6.remove();
                  }
   ...
   private static class _A {
      protected SleepClosure B;
      protected boolean A;

      public _A(SleepClosure var1, boolean var2) {
         this.B = var1;
         this.A = var2;
      }

      public SleepClosure A() {
         return this.B;
      }
